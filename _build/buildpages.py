...
FRAME = ''      # an illuminated border laid over every page of this entry (see README)
...
def build_page(doc, pno, outdir, n, title, links_out, manual=(), ocr=False):
    ...
    for inf in page.get_image_info(xrefs=True):
        ...
        # A LOOPING VIDEO IN THE PAGE (Fadak 9/18): give a picture entry in _pics/<id>.json a "video" key
        # (path relative to pages/<id>/, e.g. "../../assets/silkroad_loop.mp4") and the still in the Word layout
        # becomes the poster: a muted, looping, autoplaying <video> is laid exactly over it. Optional "webm" too.
        if p.get('video'):
            srcs = '<source src="%s" type="video/webm">' % html.escape(p['webm'], quote=True) if p.get('webm') else ''
            srcs += '<source src="%s" type="video/mp4">' % html.escape(p['video'], quote=True)
            pics_html.append('<video class="pvid" autoplay muted loop playsinline preload="metadata" poster="%s" '
                             'style="left:%.1fpx;top:%.1fpx;width:%.1fpx;height:%.1fpx"%s%s>%s</video>'
                             % (p['file'], r.x0 * SCALE, r.y0 * SCALE, r.width * SCALE, r.height * SCALE,
                                (' title="%s"' % html.escape(hover, quote=True)) if hover else '',
                                ' aria-hidden="true"' if deco else ' aria-label="%s"' % html.escape(alt, quote=True), srcs))
            continue
        # AN ANIMATED SCENE: give a picture entry in _pics/<id>.json an "anim" key naming a scene
        # ("caravan"), with optional "phase" (0 day, 1 dusk, 2 night, 3 dawn) and "terrain". The Word
        # still stays in the page art underneath, so reduce-motion readers see the picture instead.
        if p.get('anim'):
            mk = ANIMS.get(p['anim'])
            if not mk:
                print('   page %d: no animated scene called "%s" - leaving the still' % (n, p['anim']))
            else:
                extra = ((' title="%s"' % html.escape(hover, quote=True)) if hover else '') + \
                        (' aria-hidden="true"' if (deco or not alt) else
                         ' role="img" aria-label="%s"' % html.escape(alt, quote=True))
                pics_html.append(mk('%s_p%d' % (p['file'].split('.')[0], n),
                                    r.x0 * SCALE, r.y0 * SCALE, r.width * SCALE, r.height * SCALE,
                                    extra, p.get('phase', 0), p.get('terrain', 'desert')))
                continue
        if p['w'] < 1.25 * r.width * SCALE: continue      # shown at (near) full size already: nothing to grow into
        if p.get('noenlarge'): continue                # "noenlarge": true in _pics/<id>.json - nothing worth growing into
    ...
    frame_html = ''   # the illuminated border is drawn by the reader shell, over and beyond the paper
    # With a border, the paper is cut back to a rectangle that sits UNDER the ornament, so where the
    # ornament's outline is ragged the reader's sky shows through instead of a white corner. The inset
    # is a percentage so it survives the page being scaled to any size.
    if FRAME:
        clip = 8.0 if isinstance(FRAME, str) else float((FRAME or {}).get('clip', 8))
        doc_html = doc_html.replace('</style>',
            'html,body{background:transparent;overflow:hidden}\n.sheet{background:transparent}\n'
            '.page{background:#fff;clip-path:inset(%.3f%% %.3f%%)}\n</style>'
            % (clip / hmm * 100, clip / wmm * 100), 1)
...
p{margin:0;position:absolute;z-index:2}
a{color:inherit}
a:hover{text-decoration:underline}
a.pic{position:absolute;display:block;z-index:4;cursor:zoom-in;border-radius:2px}
video.pvid{position:absolute;display:block;z-index:4;object-fit:cover;border-radius:2px;background:#000}
@media (prefers-reduced-motion:reduce){video.pvid{display:none}}
svg.panim{position:absolute;display:block;z-index:1}   /* under the words, over the page art */
@media (prefers-reduced-motion:reduce){svg.panim{display:none}}
@keyframes anmFar{from{transform:translateX(-350px)}to{transform:translateX(0)}}
@keyframes anmNear{from{transform:translateX(-350px)}to{transform:translateX(0)}}
@keyframes anmPlod{0%%,100%%{transform:translateY(0)}50%%{transform:translateY(-2.5px)}}
svg.panim .sFar{animation:anmFar 30s linear infinite}
svg.panim .sNear{animation:anmNear 13s linear infinite}
svg.panim .plod{animation:anmPlod 1.5s ease-in-out infinite;transform-origin:center bottom}
'''+_notes.CSS+'''
...
@media (prefers-reduced-motion:reduce){a.pic.pulse{animation:none;box-shadow:0 0 0 3px rgba(251,170,25,.6)}}
...
# ---- AN ANIMATED SCENE LAID OVER A PICTURE ----------------------------------------------------
# The still in the Word layout is the poster; a live SVG is laid exactly over it. Vector, so it stays
# crisp at any zoom the reader offers, a few KB rather than a video file, and it works from a folder.
# The scene below is the caravan from The Silk Road (journeyScene), kept faithful to the original.
ANIM_SKY = [
    {'t': '#aac4dc', 'b': '#ead9b3', 'sun': 1,   'moon': 0,  'star': 0, 'veil': 0},      # 0 day
    {'t': '#d98e54', 'b': '#b04a2e', 'sun': .55, 'moon': 0,  'star': 0, 'veil': .12},    # 1 dusk
    {'t': '#16203c', 'b': '#2c2746', 'sun': 0,   'moon': 1,  'star': 1, 'veil': .5},     # 2 night
    {'t': '#caa0a8', 'b': '#e6c39a', 'sun': .35, 'moon': .3, 'star': 0, 'veil': .1},     # 3 dawn
]
ANIM_GROUND = {'settled': '#6a5630', 'steppe': '#5a6a3a', 'desert': '#8a6638',
               'harsh': '#7a5630', 'mountain': '#544c40', 'rugged': '#6a5a44'}

def _camel(dx, delay):
    return ('<g transform="translate(%g,0)"><g class="plod" style="animation-delay:%gs">'
            ...
            '<ellipse cx="-3" cy="4" rx="3.4" ry="2.6" fill="#34261a"/>'
            '<rect x="-8" y="3" width="4" height="2.4" rx="1" fill="#34261a"/>'
            '</g></g>') % (dx, delay)

def caravan_svg(uid, x, y, w, h, extra, phase=0, terrain='desert'):
    p = ANIM_SKY[int(phase) % len(ANIM_SKY)]
    gc = ANIM_GROUND.get(terrain, ANIM_GROUND['desert'])
    farp = 'M0 200 L0 150 Q60 124 120 146 Q190 170 260 140 Q320 120 350 144 L350 200 Z'
    nearp = 'M0 200 L0 168 Q70 144 150 166 Q230 186 300 160 Q345 148 350 166 L350 200 Z'
    # three copies of the dune band, 350 apart, so sliding it 350px left loops seamlessly
    def tile(d, op=''):
        return ''.join('<path d="' + d + '"' + t + ' fill="' + gc + '"' + op + '/>'
                       for t in ('', ' transform="translate(350,0)"', ' transform="translate(700,0)"'))
    stars = ''.join('<circle cx="%d" cy="%d" r="%s" fill="#fff"/>'
                    % ((i * 53) % 690 + 5, (i * 37) % 90 + 8, '.8' if i % 3 else '1.3') for i in range(26))
    gid = 'sky_' + uid          # ids must be unique: two scenes could share a page
    drover = ('<g class="plod" style="animation-delay:.25s">'
              '<ellipse cx="0" cy="8" rx="3.2" ry="4" fill="#241a12"/>'
              '<rect x="-2.4" y="11" width="4.8" height="15" rx="1.5" fill="#241a12"/>'
              '<rect x="-2.4" y="25" width="2.2" height="11" fill="#241a12"/>'
              '<rect x="0.4" y="25" width="2.2" height="11" fill="#241a12"/></g>')
    return ('<svg class="panim" viewBox="0 0 700 200" preserveAspectRatio="xMidYMid slice" '
            'style="left:%.1fpx;top:%.1fpx;width:%.1fpx;height:%.1fpx"%s>'
            '<defs><linearGradient id="%s" x1="0" y1="0" x2="0" y2="1">'
            '<stop offset="0" stop-color="%s"/><stop offset="1" stop-color="%s"/></linearGradient></defs>'
            '<rect width="700" height="200" fill="url(#%s)"/>'
            '<g opacity="%s">%s</g>'
            '<circle cx="585" cy="46" r="22" fill="#ffe6a0" opacity="%s"/>'
            '<g opacity="%s"><circle cx="585" cy="46" r="17" fill="#e8ecf5"/>'
            '<circle cx="592" cy="42" r="14" fill="url(#%s)"/></g>'
            '<g class="sFar">%s</g><g class="sNear">%s</g>'
            '<g transform="translate(250,120) scale(.95)">%s%s%s%s</g>'
            '<rect width="700" height="200" fill="#0a1226" opacity="%s"/></svg>'
            % (x, y, w, h, extra, gid, p['t'], p['b'], gid, p['star'], stars, p['sun'], p['moon'], gid,
               tile(farp, ' opacity=".55"'), tile(nearp),
               _camel(148, .18), _camel(92, .5), _camel(36, 0), drover, p['veil']))

ANIMS = {'caravan': caravan_svg}

APP_HTML = """<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>%(title)s &mdash; page %(n)d: %(app)s</title>
<link rel="stylesheet" href="../../assets/fonts.css">
<style>
html,body{margin:0;padding:0;background:#fff;height:100%%;overflow:hidden}
.sheet{position:absolute;left:0;top:0;right:0;bottom:0;background:#fff}
/* the beeswax frame the other pages carry in their page art, drawn here in CSS at the same inset and weight */
.frame{position:absolute;left:%(inset).2fmm;top:%(inset).2fmm;right:%(inset).2fmm;bottom:%(inset).2fmm;border:%(fw).2fpt solid %(fcol)s;box-sizing:border-box;display:flex;flex-direction:column;background:#000}
.bar{flex:0 0 auto;display:flex;align-items:baseline;gap:14px;padding:7px 12px 6px;background:#fff;border-bottom:%(fw).2fpt solid %(fcol)s;
  font:12px/1 'Liberation Serif','Times New Roman',serif;font-variant:small-caps;letter-spacing:.14em;color:%(fcol)s;white-space:nowrap;overflow:hidden}
.bar .mid{flex:1 1 auto;text-align:center;font-variant:normal;letter-spacing:0;color:#333;font-style:italic;overflow:hidden;text-overflow:ellipsis}
.bar a{color:%(fcol)s;text-decoration:none;font-variant:normal;letter-spacing:.02em}
.bar a:hover{text-decoration:underline}
.bar .fr{letter-spacing:0;font-variant:normal}
iframe.app{flex:1 1 auto;width:100%%;min-height:0;border:0;background:#000;display:block}
/* with an illuminated border the game fills the paper and this pill sits in the border's clear centre */
.pill{position:absolute;right:%(pillin).1fmm;bottom:%(pillin).1fmm;z-index:9;font:12px/1 'Liberation Serif','Times New Roman',serif;
  background:rgba(255,253,245,.82);color:#3a2a10;border:1px solid rgba(58,42,16,.35);border-radius:14px;padding:6px 11px;
  text-decoration:none;backdrop-filter:blur(2px);opacity:.75;transition:opacity .15s}
.pill:hover{opacity:1}
.pill b{font-weight:400;opacity:.6;margin-left:8px}
</style></head><body>
<div class="sheet"><div class="frame">
%(top)s
<iframe class="app" src="%(url)s" title="%(app)s" allow="fullscreen; autoplay" loading="lazy"></iframe>
</div>%(pill)s</div>
<script>
/* opened from a folder, the hosted game is not reachable offline - swap in the local copy */
(function(){ var L=%(local)s; if(L && location.protocol==='file:'){
  var f=document.querySelector('iframe.app'); if(f) f.src=L;
  var a=document.querySelector('.pill,.bar a'); if(a) a.href=L; } })();
try{ parent.postMessage({abj:'hello'},'*'); }catch(e){}
document.addEventListener('mousedown',function(){ try{ parent.postMessage({abj:'click'},'*'); }catch(e){} });</script>
</body></html>
"""
def build_app_page(outdir, n, title, app, cfg):
    """An extra last page that is a live web app (the Silk Road game, say) in an iframe, framed like the others.
    app = {"url": ..., "title": ..., "note": ..., "wmm": 210, "hmm": 297}; returns [wmm, hmm]."""
    wmm = float(app.get('wmm', 210)); hmm = float(app.get('hmm', 297))
    framed = bool(FRAME)
    bar = ('<div class="bar"><span class="fl">%(slug)s</span><span class="mid">%(note)s</span>'
           '<a href="%(url)s" target="_blank" rel="noopener" title="Open %(app)s in its own tab">%(app)s in its own window &#x2197;</a>'
           '<span class="fr">%(n)d</span></div>')
    pill = ('<a class="pill" href="%(url)s" target="_blank" rel="noopener" title="Open %(app)s in its own tab">'
            '%(app)s in its own window &#x2197;<b>%(n)d</b></a>')
    vals = dict(title=html.escape(title), n=n, app=html.escape(app.get('title', 'the game')), note=html.escape(app.get('note', '')),
                url=html.escape(app['url'], quote=True), slug=html.escape(FOLIO_SLUG), fcol=FOLIO_COLOR,
                inset=0.0 if framed else float(cfg.get('frame_inset_mm', 10)),
                fw=0.0 if framed else float(cfg.get('frame_width_pt', 1.5)),
                pillin=float(cfg.get('frame_clear_mm', 26)),
                local=json.dumps(app.get('localUrl','')))
    vals['top'] = '' if framed else bar % vals
    vals['pill'] = (pill % vals) if framed else ''
    h_extra = 'html,body{overflow:hidden}'   # the border is skipped on this page, so no clip either
    h = APP_HTML % vals
    if framed: h = h.replace('</style>', h_extra + '\n</style>', 1)
    open(os.path.join(outdir, '%d.html' % n), 'w', encoding='utf-8').write(h)
    return [round(wmm, 2), round(hmm, 2)]
...
if __name__ == '__main__':
    ...
    for a in meta:
        ...
        FRAME = a.get('frame', '')     # e.g. "../../assets/frame-silkroad.png"
        ...
        if a.get('app') and a['app'].get('url'):   # a live web page as the final page (see README, 'A web app as the last page')
            a['n'] = n + 1; a['pmm'].append(build_app_page(out, n + 1, a['title'], a['app'], cfg))
            print('           + page %d: %s (%s)' % (n + 1, a['app'].get('title', 'app'), a['app']['url']))
