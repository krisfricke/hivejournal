...
FONTS = os.path.join(site, '_build', 'fonts')   # optional; falls back to the system Carlito
def f(name, size):
...
bee = Image.open(os.path.join(site, '_build', 'bee-original.gif')).convert('RGBA')
...
seasons = sorted(set(a['vol'] for a in meta))
y = 170
d.text((x, y), 'LJ Idol entries by', font=fit('LJ Idol entries by', 'Carlito-Regular.ttf', 44, avail), fill=INK2); y += 52
...
sub = ('%d entries' % len(meta) if len(meta) != 1 else 'One entry so far') + ', laid out as magazine pages'
ft, lines = wrap(sub, 'Carlito-Regular.ttf', 32, avail)
for ln in lines: d.text((x, y), ln, font=ft, fill=NAVY); y += ft.size + 9
y += 8
ft, lines = wrap('Season' + ('s ' if len(seasons) > 1 else ' ') + ', '.join(str(s) for s in seasons) + '  \u00b7  emo_snal on LiveJournal  \u00b7  aggienaut on Dreamwidth', 'Carlito-Regular.ttf', 24, avail)
